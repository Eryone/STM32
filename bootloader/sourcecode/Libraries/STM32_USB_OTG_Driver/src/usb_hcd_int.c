...
uint32_t USBH_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsHostMode(pdev))
  {
    ...
    if (gintsts.b.portintr)//主机检测到设备插入
    {
      retval |= USB_OTG_USBH_handle_port_ISR (pdev);//有USB 设备插入时
    }
    
    if (gintsts.b.disconnect)//检测设备被拔出
    {
...
static uint32_t USB_OTG_USBH_handle_hc_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  * GINTSTS */
  ...
  haint.d32 = USB_OTG_ReadHostAllChannels_intr(pdev);//主机所有通道中断寄存器  对应16个通道
  
  for (i = 0; i < pdev->cfg.host_channels ; i++)//遍历所有通道一个8个
  {
    if (haint.b.chint & (1 << i)) //判断此通道是否产生中断
    {
      hcchar.d32 = USB_OTG_READ_REG32(&pdev->regs.HC_REGS[i]->HCCHAR);//读取主通道x特性寄存器，
      
      if (hcchar.b.epdir)  //判断端点的传输方向  1:IN    0:OUT
      {
...
static uint32_t USB_OTG_USBH_handle_port_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  hprt0.d32 = USB_OTG_READ_REG32(pdev->regs.HPRT0);  //主机端口控制和状态寄存器
  ...
  if (hprt0.b.prtconndet)  //当检测到设备链接 此位置一
  {
  ...
  /* Port Enable Changed */
  if (hprt0.b.prtenchng)    //设备连接和断开到会置1
  {
    hprt0_dup.b.prtenchng = 1;
    
    if (hprt0.b.prtena == 1) //端口状态使能
    {
      if ((hprt0.b.prtspd == HPRT0_PRTSPD_LOW_SPEED) ||
          (hprt0.b.prtspd == HPRT0_PRTSPD_FULL_SPEED))
      { 
        hcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.HREGS->HCFG); //主机模式配置寄存器
        
        if (hprt0.b.prtspd == HPRT0_PRTSPD_LOW_SPEED)  //判断是全速还是低速
        {
      ...
      USBH_HCD_INT_fops->DevPortEnabled(pdev);  //端口使能状态置1
    ...
    {
      USBH_HCD_INT_fops->DevPortDisabled(pdev);  //设备断开连接
...
uint32_t USB_OTG_USBH_handle_hc_n_In_ISR (USB_OTG_CORE_HANDLE *pdev , uint32_t num)
{
  ...
  hcreg = pdev->regs.HC_REGS[num]; 
  hcint.d32 = USB_OTG_READ_REG32(&hcreg->HCINT); //读取主机通道x 中断寄存器
  hcintmsk.d32 = USB_OTG_READ_REG32(&hcreg->HCINTMSK);  //读取主机通道x屏蔽了那些中断
  hcint.d32 = hcint.d32 & hcintmsk.d32;  //去掉屏蔽的中断，获取中断的状态
  hcchar.d32 = USB_OTG_READ_REG32(&pdev->regs.HC_REGS[num]->HCCHAR);  //读取主机通道x特性值
  hcintmsk.d32 = 0;  
  
  if (hcint.b.ahberr)
  {
    CLEAR_HC_INT(hcreg ,ahberr);  //清楚中断
    UNMASK_HOST_INT_CHH (num);  //不屏蔽通道中止中断
  }  
  else if (hcint.b.ack)
  {
    CLEAR_HC_INT(hcreg ,ack);  //清除应答中断
  }
