               ...
               USBH_Usr_cb_TypeDef *usr_cb)
{
  ...
  /* Hardware Init 初始化USB时钟引脚中断等*/
  USB_OTG_BSP_Init(pdev);  
  
  /* configure GPIO pin used for switching VBUS power//VBUS引脚初始化 */
  USB_OTG_BSP_ConfigVBUS(0);  
  
  
  /* Host de-initializations 参数初始化*/
  USBH_DeInit(pdev, phost);
  
  /*Register class and user callbacks 回调函数赋值地址*/
  ...
  /* Enable Interrupts USB中断使能，优先级配置*/
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_IDLE :  //无USB 设备插入会一直在该状态
    
    if (HCD_IsDeviceConnected(pdev))  //检查设备是否连接，已连接，复位设备
    {
      ...
      /* User RESET callback*/
      phost->usr_cb->ResetDevice();//用户自定义复位设备函数
    }
    break;
    
  case HOST_WAIT_PRT_ENABLED:   //检测到有USB 设备插入
  ...
  case HOST_DEV_ATTACHED :  //主机检测到设备后，打开输入输出控制通道
    //为设备的双向0端点各分配一个channel，并打开
    phost->usr_cb->DeviceAttached();//用户自定义显示设备连接信息函数
    ...
    if ( HCD_ResetPort(pdev) == 0)
    {
      ...
      phost->device_prop.speed = HCD_GetCurrentSpeed(pdev);//获取设备速度,中断ISR中读取寄存器
      
      phost->gState = HOST_ENUMERATION;
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);//显示是高速，全速还是低速设备
  ...
  case HOST_USR_INPUT:    //cdc类初始化
    /*The function should return user response true to move to class state */
    if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK)
    {//为后续发起类相关传输做准备Init (pdev, phost)
     ...
     // Send_Data_ToWifi("\nconnect to TP-LINK_3928:JIETAI007;17S120TOR20039;192.168.0.230;\r\n",65); //set wifi
     // Send_Data_ToWifi("\nconnect to TP-LINK_3928:JIETAI007;17S120TOR20039;192.168.1.230;\r\n",65);
    }   
    break;
