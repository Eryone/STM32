...
{
  HOST_State            gState;       /*  Host State Machine Value ,USBH_Process大循环的switch */
  HOST_State            gStateBkp;    /* backup of previous State machine value */
  ENUM_State            EnumState;    /* Enumeration state Machine ,用于USBH_HandleEnum循环*/
  CMD_State             RequestState;  /*USBH_CtlReq循环 */    
  USBH_Ctrl_TypeDef     Control;
  
  USBH_Device_TypeDef   device_prop; 
  
  USBH_Class_cb_TypeDef               *class_cb;   /*MSC、 HID类的回调函数*/
  USBH_Usr_cb_TypeDef  	              *usr_cb;  /*用户回调函数*/
