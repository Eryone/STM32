...
#define  USB_REQ_TYPE_STANDARD                          0x00//标准种类
#define  USB_REQ_TYPE_CLASS                             0x20//类
#define  USB_REQ_TYPE_VENDOR                            0x40//厂商
#define  USB_REQ_TYPE_RESERVED                          0x60//保留

/* bmRequestType D4..0 Recipient接受者 */
#define  USB_REQ_RECIPIENT_DEVICE                       0x00//设备
#define  USB_REQ_RECIPIENT_INTERFACE                    0x01//接口
#define  USB_REQ_RECIPIENT_ENDPOINT                     0x02//端点
#define  USB_REQ_RECIPIENT_OTHER                        0x03//其他4-32保留
...
typedef struct _ConfigurationDescriptor
{
  ...
  uint8_t   bNumInterfaces;       /* Number of Interfaces 此配置所支持的接口数量*/
  uint8_t   bConfigurationValue;  /* Value to use as an argument to select this configuration Set_Configuration命令所需要的参数值*/
  uint8_t   iConfiguration;       /*Index of String Descriptor Describing this configuration 描述该配置的字符串的索引值*/
  uint8_t   bmAttributes;         /* D7 Bus Powered , D6 Self Powered, D5 Remote Wakeup , D4..0 Reserved (0)供电模式的选择*/
  uint8_t   bMaxPower;            /*Maximum Power Consumption 设备从总线提取的最大电流*/
}
...
typedef struct _InterfaceDescriptor
{
  ...
  uint8_t bNumEndpoints;        /* Number of Endpoints used for this interface 该接口使用的端点号，不包括端点0*/
  uint8_t bInterfaceClass;      /* Class Code (Assigned by USB Org) 接口类型*/
  uint8_t bInterfaceSubClass;   /* Subclass Code (Assigned by USB Org) 接口子类型*/
  uint8_t bInterfaceProtocol;   /* Protocol Code 接口遵循的协议*/
  uint8_t iInterface;           /* Index of String Descriptor Describing this interface 描述该接口的字符串索引值*/
...
typedef struct _EndpointDescriptor
{
  uint8_t   bLength;
  uint8_t   bDescriptorType;
  uint8_t   bEndpointAddress;   /* indicates what endpoint this descriptor is describing 端点地址及输入输出属性*/
  uint8_t   bmAttributes;       /* specifies the transfer type. 端点的传输类型属性*/
  uint16_t  wMaxPacketSize;    /* Maximum Packet Size this endpoint is capable of sending or receiving 端点收、发的最大包大小*/  
