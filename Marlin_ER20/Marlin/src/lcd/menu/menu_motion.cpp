...
extern char adust_z_ing;
static void save_adjust_z()
{
  probe.offset.z=probe.offset.z+current_position[Z_AXIS];
  lcd_store_settings();
 // settings.save();
...
void select_adjust_z()
{
    MenuItem_confirm::select_screen(
            PSTR("YES"), PSTR("NO"),
            save_adjust_z, ui.goto_previous_screen,
            PSTR(""), " Save Z value ", PSTR("?")
          );
...
static void _lcd_move_xyz(PGM_P const name, const AxisEnum axis) {
  ...
  { 
    ...
    if(adust_z_ing==1)
         adust_z_ing=2;
  	else if(adust_z_ing==0)
       return  ui.goto_previous_screen_no_defer();
    }
  }
else if(adust_z_ing==2)
{ 
    	  select_adjust_z();
        return ;
  ...
  {    
    START_SCREEN();
    STATIC_ITEM_P("Move Z until nozzle",SS_LEFT);                                          // 2: Message Line 1
    STATIC_ITEM_P("touches the bed",SS_LEFT);   
    END_SCREEN();
  }
