...
void menu_calibrate_waiting()
{
  PGM_P const msg1 = "Waiting...";
  START_SCREEN();
  //if (skip1) SKIP_ITEM();                                       // Move a single-line message down
  STATIC_ITEM_P(msg1);                                          // 2: Message Line 1
  END_SCREEN();
}
void calibrate_z_move()
{
  adust_z_ing=1;
  ui.defer_status_screen();
  move_menu_scale = 0.1;
  ui.goto_screen(menu_calibrate_waiting);
  ui.use_click();
...
//
// About Printer > printer self test
//
void menu_calibrate_z() {
 MenuItem_confirm::select_screen(
            PSTR("YES"),PSTR("NO"),
            calibrate_z_move, ui.goto_previous_screen,
            PSTR("Start to calibrate "), PSTR(" the z probe offset"), PSTR("?")
          );
...
void menu_configuration() {
  ...
  #endif
  SUBMENU(MSG_SELF_TEST, menu_calibrate_z);               // Self Test >
  //
